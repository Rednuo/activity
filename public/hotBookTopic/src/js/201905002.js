...
new Vue({
    ...
    methods: {
        ...
        getBookInfo(){
            let self = this;
            axios.get(`${url.getBookPriceInfo}5c6fd0eb7af6804ec436941c/price-info?platform=${self.platform}`).then((res) => {
                res = res.data;
                if (res && res.ok) {
                    console.log(res)
                    if(res.doc.discount){
        ...
        getBookList() {
            let self = this;
            axios.get(`${url.getBookListNode}40d31ab3861d4c2f9e9894032ec541d6`).then((res) => {
                res = res.data;
                if (res && res.ok) {
                    console.log(res)
