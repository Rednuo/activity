...
new Vue({
    ...
    mounted: function () {
        this.umeng('201905012访问')
        HybridApi.setUserBehavior(`f02##-1|201905012`);
        this.init();
    },
    methods: {
        init(){
        ...
        getBookList() {
            let self = this;
            axios.get(`${url.getBookListNode}594815f75f03435aa7dac2dd345c2880`).then((res) => {
                res = res.data;
                if (res && res.ok) {
                    self.bookList = res.data.books.splice(0,3);
                }
            });
        },
        batchAddBookShelf() {
