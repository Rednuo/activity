...
new Vue({
    ...
    created: function () {
        ...
        if (self.containsPackageName.indexOf(self.packageName) == -1) {
            location.replace(location.href.replace('index.html', 'index_old.html'));
        }
        HybridApi.init();
    ...
    computed: {
        ...
        hideServiceBtn() {
            var self = this
            return !!self.deviceInfo && self.deviceInfo.channel == 'Vivo' && self.deviceInfo.versionCode >= '5801'
        }
    },
    mounted: function () {
        var self = this;
        HybridApi.getDeviceInfo(function (deviceInfo) {
            ...
            self.currentVersion = deviceInfo.version.split('.');
        })
