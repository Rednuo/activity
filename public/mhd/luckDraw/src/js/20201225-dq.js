...
const vm = new Vue({
    ...
    methods: {
        ...
        Sign:function (item, isrepairsign) {
            ...
            if (isrepairsign) {
                tumeng.push(['2020双旦活动-大全','点击签到']);
            } else {
                tumeng.push(['2020双旦活动-大全','点击补签']);
            }
            return new Promise((resolve, reject) => {
                axios({
                    method: 'POST',
            ...
            return new Promise((resolve, reject) => {
                axios({
                    ...
                    if (data.data.code == 200) {
                        ...
                        self.getPrize = true;
                        if(info){
                            tumeng.push(['2020双旦活动-大全','瓜分奖励弹窗']);
                        }else{
                            tumeng.push(['2020双旦活动-大全','无奖励']);
                        }
        ...
        onClickCard(item) {
            let self = this;
            if(new Date('2021/01/03 00:00:00') > self.currentTime){
                if (!self.isSignUp) {
                    Toast.show('需要先报名活动')
                    return
                }
