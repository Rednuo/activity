...
const vm = new Vue({
    ...
    methods: {
            ...
            axios.get(config.getServerTime + '&t=' + new Date().getTime()).then((res) => {
                if (res.data.code == 200) {
                    ...
                    setInterval(() => {
                        ...
                        self.currentDate = utils.getDate(self.currentTime);
                        if(new Date('2021/02/21 20:08:00') < self.currentTime){
        ...
        goLottery:function () {
            ...
            if(new Date('2021/02/21 20:08:00') > self.currentTime){
                HybridUtil.request({
                    ...
                    param: {
                        ...
                        'fullscreen': 1,
                        'title':'抽奖',
                        'link':'http://m.manhuadao.cn/activity/mhd-lottery/indexdq.html'
                    },
                    callback:function () {
                        self.freshen=false;
                        self.init();
            ...
            tumeng.push(['2021新春活动-大全','点击去抽奖']);
        },
        openTaskCenter:function () {
            HybridUtil.request({
                action: 'taskCenter',
                param: {
                    stat_nav: '任务中心',
        ...
        Sign:function (item, isrepairsign) {
            ...
            if (isrepairsign) {
                tumeng.push(['2021新春活动-大全','点击签到']);
            } else {
                tumeng.push(['2021新春活动-大全','点击补签']);
            }
            return new Promise((resolve, reject) => {
                axios({
                    method: 'POST',
            ...
            return new Promise((resolve, reject) => {
                axios({
                    ...
                    if (data.data.code == 200) {
                        ...
                        self.getPrize = true;
                        if(info){
                            tumeng.push(['2021新春活动-大全','瓜分奖励弹窗']);
                        }else{
                            tumeng.push(['2021新春活动-大全','无奖励']);
                        }
        ...
        onClickCard(item) {
            let self = this;
            if(new Date('2021/02/21 00:00:00') > self.currentTime){
                if (!self.isSignUp) {
                    Toast.show('需要先报名活动')
                    return
                }
                console.log(item.date)
                console.log(new Date(item.signtime.replace(/-/g, "/")))
